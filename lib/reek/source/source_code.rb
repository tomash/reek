      ...
      def syntax_tree
        @syntax_tree ||=
          begin
            ...
            # The AST (Parser::AST::Node) at this stage would look like the one in the method
            # documentation above (but without the SexpExtensions which will be added next).
            # A comment_map would look like
            # { (class (const nil :C).... =>
            #     [#<Parser::Source::Comment string:1:9 "# comment about C">]}
            # With the key being the AST and the value an array with all relevant comments.
            # Finally we'll pass that to our TreeDresser and return that.
